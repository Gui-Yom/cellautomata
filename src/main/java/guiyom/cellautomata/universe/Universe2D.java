package guiyom.cellautomata.universe;
...
public class Universe2D {
    ...
    public void nextGen() {
        ...
        for (int i = 0; i < width; ++i)
            for (int j = 0; j < height; ++j)
                set(i, j, rule.apply(get(i, j), countNeighbours(i, j)) ? CellAutomata.ALIVE : CellAutomata.DEAD);
        swapBuffers();
    }
    ...
    public int countNeighbours(int x, int y) {
        ...
        for (int i = -1; i <= 1; ++i)
            for (int j = -1; j <= 1; ++j)
                if (i != 0 || j != 0)
                    if (get(x + i, y + j) == CellAutomata.ALIVE)
                        ++count;
    ...
    public void drawLine(int x1, int y1, int x2, int y2) {
        ...
        for (int i = 0; i <= distance; ++i) {
            ...
            read[(int) (Math.round(x1 + t * (x2 - x1)) * width + Math.round(y1 + t * (y2 - y1)))] = CellAutomata.ALIVE;
        }
    }

    public void forEach(CellConsumer transform) {

        for (int i = 0; i < getWidth(); ++i)
            for (int j = 0; j < getHeight(); ++j)
                transform.apply(get(i, j), i, j);
    }
